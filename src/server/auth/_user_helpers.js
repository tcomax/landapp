...
function updateProfile(req, res) {
  ...
  if (req.file) {
    ...
    console.log(`fileType(picture) ${picture.mimetype}`);
    if (picture.mimetype === 'image/png') {
      pictureFileExt = '.png';
    } else if (picture.mimetype === 'image/jpeg') {
      pictureFileExt = '.jpg';
    }
    let pictureFileName = `${userId}${pictureFileExt}`;
    let pictureS3FileName = `media/pictures/${d.toLocaleDateString()}/${pictureFileName}`;
    let uploadParams = {
      Bucket: 'landswoop',
      Key: pictureS3FileName,
      Body: picture.buffer,
      ACL: 'public-read'
    };
    console.log(`pictureS3FileName ${pictureS3FileName}`);
    return S3.upload(uploadParams, function (err, data) {
      if (err) {
        ...
        .returning('cntct_id');
        console.log(`sess: ${query}`);
        return query.then((cntct_id) => {
...
function getProfile(req, res) {
  ...
  .join('contacts', function() {
    this.on('user_info.cntct_id', '=', 'contacts.id')
      .andOn('user_info.id', '=', userId);
  })
  .select(
    'user_info.id',
    'name',
  ...
  return query.then((userInfo) => {
    ...
    res.status(200).json({data: sess.userInfo, status: 'success'});
  }).catch((err) => {
    res.status(400).json({status: err.message});
    req.logout();
  });
...
function findContactByPhone(req, res) {
  ...
  return query.then((cntctInfo) => {
    // console.log(`contact: ${JSON.stringify(cntctInfo)}`);
    if (cntctInfo[0] === undefined) {
      sess.cntctInfo = 'none';
    } else {
      sess.cntctInfo = cntctInfo[0];
    }
    // console.log(`sess: ${JSON.stringify(sess)}`);
    res.status(200).json(sess.cntctInfo);
  }).catch((err) => {
    res.status(400).json({status: err.message});
  });
}
...
function getFeeds(req, res) {
  ...
  let query = baale('feeds')
  .select('*')
  .innerJoin('user_info','feeds.user_id','user_info.id')
  .orderBy('timestmp', 'desc')
  .limit(limit)
  .offset(offset)
  .returning('*');
  // console.log(query.toString());
  return query.then((feeds) => {
    if (feeds === undefined) {
      sess.feeds = 'none';
    } else {
      sess.feeds = feeds;
    }
    //console.log(`feeds: ${JSON.stringify(sess.feeds)}`);
    res.status(200).json(sess.feeds);
  }).catch((err) => {
    res.status(400).json({status: err.message});
  });
}
